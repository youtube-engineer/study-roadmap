  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={title}
        aria-modal={open}
        className={`absolute bottom-0 left-1/2 flex max-h-[82vh] w-[min(480px,100%)] flex-col rounded-t-[18px] border-t border-rule bg-raised pb-[env(safe-area-inset-bottom)] shadow-lift transition-transform duration-[280ms] ease-[cubic-bezier(0.2,0,0,1)] ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 pb-2.5 pt-2 text-[0.95rem] font-bold text-ink-title">
          <span>{title}</span>
          <button
            type="button"
            onClick={onClose}
